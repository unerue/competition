...
def byhand(dfs):
    ...
    mtot=0
    with open('input/df_all.bin', 'wb') as f:
        for df in dfs:
            m, n = df.shape
            mtot += m
            ...
            typ = df.values.dtype                
    #del dfs
    with open('input/df_all.bin','rb') as f:
        buffer = f.read()
        data = np.frombuffer(buffer, dtype=typ).reshape(mtot, n)
        # combined_df = pd.DataFrame(data=data, columns=col_names)
        combined = pd.DataFrame(data, columns=col_names)
        
    os.remove('input/df_all.bin')
...
def data_loader_all(func, path, train, nrows, **kwargs):
    ...
    # 읽어올 파일들만 경로 저장 해놓기 
    files_in_dir = os.listdir(path)[:50]
    ...
    cdf_df = byhand(df_list)
#     cdf_df = cudf.DataFrame.from_pandas(byhand(df_list))
